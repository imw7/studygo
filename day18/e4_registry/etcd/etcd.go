...
func init() {
	...
	if err := registry.RegisterPlugin(etcdRegistry); err != nil {
		err = fmt.Errorf("init registerPlugin error, err:%v\n", err)
	}
	go etcdRegistry.run()
}
...
func (e *EtcdRegistry) syncServiceFromEtcd() {
	...
	for _, service := range allServiceInfo.serviceMap {
		...
		for _, kv := range resp.Kvs {
			...
			if err = json.Unmarshal(value, &tmpService); err != nil {
				fmt.Printf("unmarshal failed, err:%v value:%s\n", err, string(value))
				return
			}
...
func (e *EtcdRegistry) registerOrKeepAlive() {
	for _, registryService := range e.registryServiceMap {
		...
		if err := e.registerService(registryService); err != nil {
			fmt.Println("register error, err:", err)
		}
	}
}
...
func (e *EtcdRegistry) Init(ctx context.Context, opts ...registry.Option) (err error) {
	...
	if err != nil {
		err = fmt.Errorf("init etcd error, err:%v\n", err)
		return
	}
	return
}
